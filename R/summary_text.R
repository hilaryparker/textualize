...
summary_text <- function(test, alpha=0.05){
  ...
  if (test$p.value < alpha) {
    stmt <- paste0(
      stmt, "reject the null hypothesis, and conclude that the true population ",
      "proportion is different than "
    )
  } else {
    stmt <- paste0(
      stmt, "do not reject the null hypothesis, and cannot conclude that the ",
      "true population proportion is different than "
    )
  }

  stmt <- paste0(
    stmt, test$null.value, ". The observed number of events is ",
